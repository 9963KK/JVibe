...
export const JVIBE_RUNTIME_PACKAGE = "jvibe-runtime";
...
export function findPiPackageRootFromEntry(entryPoint: string): string | undefined {
	...
	while (dir !== path.dirname(dir)) {
		...
		if (fs.existsSync(packageJsonPath)) {
			...
			if (pkg.name === PI_CODING_AGENT_PACKAGE || pkg.name === JVIBE_RUNTIME_PACKAGE) return dir;
		}
...
export function resolvePiPackageRoot(): string | undefined {
	const jvibePackageDir = process.env.JVIBE_PACKAGE_DIR?.trim();
	if (jvibePackageDir && fs.existsSync(path.join(jvibePackageDir, "package.json"))) {
		return fs.realpathSync(jvibePackageDir);
	}
	try {
...
export function resolveWindowsPiCliScript(deps: PiSpawnDeps = {}): string | undefined {
	...
	try {
		...
		const binPath = typeof binField === "string"
			? binField
			: binField?.jvibe ?? binField?.pi ?? Object.values(binField ?? {})[0];
		...
		// CLI resolution is optional; falling back to `jvibe` lets PATH handle execution.
		return undefined;
	}
...
export function getPiSpawnCommand(args: string[], deps: PiSpawnDeps = {}): PiSpawnCommand {
	...
	if (platform === "win32" || process.env.JVIBE_PACKAGE_DIR) {
