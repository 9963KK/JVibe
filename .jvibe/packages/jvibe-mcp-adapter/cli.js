...
const AGENT_DIR = process.env.JVIBE_CODING_AGENT_DIR?.trim()
  ? expandHome(process.env.JVIBE_CODING_AGENT_DIR.trim())
  : path.join(HOME, ".jvibe", "agent");
...
const PROJECT_PI_CONFIG_PATH = path.resolve(process.cwd(), ".jvibe", "mcp.json");
...
function printHelp(log = console.log) {
  log("jvibe MCP adapter helper\n");
  log("Install the package with:");
  log("  jvibe install npm:pi-mcp-adapter\n");
  log("Then optionally run:");
  log("  pi-mcp-adapter init       Detect host configs and scaffold JVibe imports");
...
function printDiscovery(log, imports) {
  ...
  const paths = [
    ...
    ["JVibe global override", PI_CONFIG_PATH],
    ["Project standard MCP", PROJECT_CONFIG_PATH],
    ["Project JVibe override", PROJECT_PI_CONFIG_PATH],
  ];
...
async function runInit(argv, log = console.log) {
  ...
  if (importsToAdd.length === 0) {
    log("\nNo JVibe config changes needed.");
  ...
  log(`\nDetected host configs to import into JVibe: ${importsToAdd.join(", ")}`);
  ...
  log("JVibe will now keep reading standard MCP configs automatically, while these imports cover host-specific config formats.");
  return 0;
}
