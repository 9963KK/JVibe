...
export class McpSetupPanel {
  ...
  private getActions(): Action[] {
    ...
    if (this.discovery.imports.length > 0) {
      actions.push({ id: "adopt-imports", label: "Adopt detected compatibility imports", description: `Choose which host-specific MCP configs JVibe should import into its own override file. ${this.discovery.imports.length} source${this.discovery.imports.length === 1 ? "" : "s"} found.` });
    }
    ...
    if (!this.discovery.sources.some((source) => source.id === "shared-project" && source.exists)) {
      actions.push({ id: "scaffold-project", label: "Scaffold project `.mcp.json`", description: "Write a minimal project config using the standard shared MCP file path, then reload JVibe." });
    }
    actions.push({ id: "show-precedence", label: "Explain config precedence", description: "Show the read order and where JVibe writes compatibility settings." });
    if (this.getDetectedPaths().length > 0) {
      actions.push({ id: "open-paths", label: "Open detected config paths", description: "Browse the actual config files that JVibe discovered on this machine." });
    }
  ...
  private async runAction(action: ActionId): Promise<void> {
    ...
    if (action === "scaffold-project") {
      await this.runBusy(async () => {
        ...
        this.notice = { text: `Wrote starter config to ${result.path}. JVibe will reload after this panel closes.`, tone: "success" };
      });
      return;
    }
    if (action === "add-repoprompt") {
      await this.runBusy(async () => {
        ...
        this.callbacks.markSetupCompleted();
        this.notice = { text: `Added ${result.serverName} to ${result.path}. JVibe will reload after this panel closes.`, tone: "success" };
      });
      return;
    }
    if (action === "close") {
  ...
  private async applySelectedImports(): Promise<void> {
    ...
    await this.runBusy(async () => {
      ...
      this.notice = result.added.length > 0
        ? { text: `Added ${result.added.join(", ")} to ${result.path}. JVibe will reload after this panel closes.`, tone: "success" }
  ...
  private discoverySummaryLine(): string {
    ...
    if (this.discovery.totalServerCount === 0 && (this.discovery.imports.length > 0 || !!this.discovery.repoPrompt.executablePath)) {
      return fg(this.t.warning, "JVibe found MCP-related setup options, but none are active in JVibe yet.");
    }
    ...
    return fg(this.t.hint, `Detected ${this.discovery.totalServerCount} configured servers across ${shared} shared and ${piOwned} JVibe-owned source${shared + piOwned === 1 ? "" : "s"}.`);
  }
  ...
  private secondarySummaryLine(): string {
    ...
    if (this.discovery.totalServerCount === 0 && this.discovery.imports.length > 0) {
      return `Detected ${this.discovery.imports.length} compatibility import source${this.discovery.imports.length === 1 ? "" : "s"}. Adopt them into JVibe or inspect the underlying files.`;
    }
    return "Shared MCP files are preferred. JVibe-owned files are only for compatibility imports and adapter-specific overrides.";
  }
    ...
    switch (action) {
      ...
      case "adopt-imports":
        return this.formatWritePreview(
          ...
          [
            ...
            "Selected imports are written into the JVibe agent dir config as JVibe-owned compatibility state.",
          ],
        );
      case "view-example":
        return this.formatPreview([
      ...
      case "show-precedence":
        return this.formatPreview([
          ...
          "2. <JVibe agent dir>/mcp.json",
          "3. .mcp.json",
          "4. .jvibe/mcp.json",
          "JVibe writes compatibility imports and adapter-only overrides to JVibe-owned files.",
        ]);
      case "open-paths":
