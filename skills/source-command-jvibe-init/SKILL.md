---
name: "source-command-jvibe-init"
description: "初始化 JVibe 项目文档结构"
---

# source-command-jvibe-init

Use this skill when the user asks to run the migrated source command `JVibe:init`.
...
当项目已有代码或文档（如存在 `README.md`、`package.json`、`src/`、`docs/project/**`）时，**优先进入扫描模式**：

1. **快速扫描现有项目**（避免 `node_modules/` 等大目录）：
   - 入口：`README.md`、`package.json`、`docs/project/**`
   - 代码结构：`src/`、`app/`、`lib/`、`cmd/`
   - API/路由线索：`routes/`、`controllers/`、`handlers/`
2. **生成项目快照**：
   - 技术栈与运行方式
   - 模块清单（基于目录/路由/服务名称归类）
   - 关键功能列表（按模块汇总）
3. **用快照填充文档**：
   - Project.md：模块清单、依赖关系、技术栈
   - Feature-List.md：为每个模块生成 3~8 个功能条目
4. **仅在信息缺失时询问用户**（确认模块边界、领域术语）

**功能条目生成规则（扫描模式）**：
- 已存在的功能可标记为 ✅，并在 TODO 中写入**已勾选**的“导入自存量功能”项，以保证状态一致
- 不确定的功能标记为 🚧，并添加 TODO：`- [ ] 确认现状/补齐测试`
...
如果检测到已有项目代码/文档，先输出“扫描结果快照”，并请求用户确认后再写入文档。
...
6. 自动识别项目路径并写入 Project.md：
   ```yaml
   detect_paths:
     project_root: $PWD (包含 .git 或 .Codex)
