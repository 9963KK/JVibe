...
| Visual workspace | 吸收 Craft Agents 的多会话、source、permission、diff、artifact 体验 |
...
- 保持 `jvibe` 为默认 TUI 入口。
- 新增 `jvibe visual` 作为可视化工作台入口。
- 参考 `craft-agents-oss` 的 Electron/Web UI、session inbox、sources、permission mode、multi-file diff 和 background tasks。
- 优先实现 JVibe runtime bridge，避免 UI 直接拥有 Agent 调度规则。
...
| Mode | 命令 | 用法 |
|------|------|------|
| TUI Mode | `jvibe` | 日常 coding、快速执行、终端内工作 |
| Visual Mode | `jvibe visual` | 多会话、source 管理、diff/artifact review、长任务管理 |

两种模式共享 `kernel/`、`.jvibe/`、`.mcp.json` 和 `state/`。Visual Mode 不 fork 内核，只通过 runtime bridge 读取 session event stream 并发送用户意图。
